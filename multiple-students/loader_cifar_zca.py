...
class CIFAR10(data.Dataset):
    ...
    def __init__(self, root, split='train',
        ...
        for fentry in self.train_list:
            ...
            self.data['train_x'].append(entry['data'])

            if 'labels' in entry:
                self.data['train_y'] += entry['labels']
            else:
                self.data['train_y'] += entry['fine_labels']
            fo.close()

        self.data['train_x'] = np.concatenate(self.data['train_x'])
        self.data['train_x'] = self.data['train_x'].reshape((len(self.data['train_x']), 3, 32, 32))
        ...
        if sys.version_info[0] == 2:
            ...
            entry = pickle.load(fo, encoding='latin1')
        self.data['test_x'] = entry['data']
        if 'labels' in entry:
            self.data['test_y'] = entry['labels']
        else:
            self.data['test_y'] = entry['fine_labels']
        fo.close()
        ...
        if self.split is 'label' or self.split is 'unlabel' or self.split is 'valid':
            ...
            self.train_data = self.data['train_x'].astype(np.float32).transpose(0,2,3,1)
            #self.train_data = np.concatenate(self.train_data)
            self.train_labels = self.data['train_y']
            ...
            self.test_labels = self.data['test_y']
...
if __name__ == '__main__':
    ...
    labelset = CIFAR10('./dataset', split='label', download=False, transform=None, boundary=0)
    unlabelset = CIFAR10('./dataset', split='unlabel', download=False, transform=None, boundary=0)
