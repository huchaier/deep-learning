...
class TfrecordLoader:
    ...
    def load_dataset(self):
        ...
        def __tfrecord_parser(sample):
            ...
            # without loosing the shuffle batches   仅适应于时序模型
            keys_to_features = {
            ...
            parsed_features = tf.parse_single_example(sample, keys_to_features)   # 返回一个字典，映射功能键到Tensor
            image = tf.reshape(parsed_features['image'], self._image_size)
            label = tf.one_hot(tf.cast(parsed_features['label'], tf.int64), self._num_classes) # 输出onehot值
