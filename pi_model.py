...
def temporal_ensembling_loss(X_train_labeled, y_train_labeled, X_train_unlabeled, model, unsupervised_weight, ensembling_targets):
    ...
    Arguments:
        ...
        unsupervised_weight {float} -- 无监督损失权重w(t)
        ensembling_targets {np.array} --  上一批次的zi

    Returns:
        {tensor} -- predictions for the ensembles：集合的预测
        {tensor} -- loss value：损失值
    """
...
def temporal_ensembling_gradients(X_train_labeled, y_train_labeled, X_train_unlabeled, model, unsupervised_weight, ensembling_targets):
    """ Gets the gradients for the temporal ensembling model 得到时序组合模型的梯度
    ...
    Returns:
        ...
        {tensor} -- loss value
        {tensor} -- gradients for each model variables 每个模型变量的梯度
    """
    # tape的用法详见：https://blog.csdn.net/xierhacker/article/details/53174558
...
def pi_model_loss(X_train_labeled, y_train_labeled, X_train_unlabeled,
    ...
    """ Gets the Loss Value for SSL Pi Model pi模型的损失值

    Arguments:
        X_train_labeled {tensor} -- train images  batch_size*32*32*3
        y_train_labeled {tensor} -- train labels
        X_train_unlabeled {tensor} -- unlabeled train images   batch_size*32*32*3
    ...
    z_labeled = pi_model(X_train_labeled)   # 有监督第一遍
    z_labeled_i = pi_model(X_train_labeled)   # 第二遍

    z_unlabeled = pi_model(X_train_unlabeled)  # 无监督第一遍
    z_unlabeled_i = pi_model(X_train_unlabeled)  # 第二遍
...
def ramp_up_function(epoch, epoch_with_max_rampup=80):
    """ Ramps the value of the weight and learning rate according to the epoch
        ...
        function=exp{-5*(1-epoch/80)*(1-epoch/80)}
...
def ramp_down_function(epoch, num_epochs):
    """ Ramps down the value of the learning rate and adam's beta
        in the last 50 epochs according to the paper 降低最近50epoch的学习率
...
class PiModel(tf.keras.Model):
    ...
    def __init__(self):
        """ Init
            ...
            to be stored)
            设置在梯度下降过程中需要跟踪的所有图层（例如，不需要储存的pool和dropout）
            tf.keras.initializers.he_uniform():均匀方差缩放初始化器
            构建网络的模型
        """
    ...
    def __apply_image_augmentation(self, image):
        ...
        输入数据的增强
        Arguments:
            image {tensor} -- image shape=(?, 32, 32, 3)
    ...
    def call(self, input, training=True):
        ...
        # Average Pooling
        h = tf.reduce_mean(h, reduction_indices=[1, 2])   # 1.2的维度和定义的Tensor有关
