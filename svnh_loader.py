...
class SvnhLoader:
    """ If not in the disk, download the SVNH dataset and prepares the dataset ready
        ...
        如果不在磁盘中，请下载SVNH数据集，并准备好准备好为张量流中的半监督学习任务加载该数据集，
    """
    ...
    def __init__(self, dataset_path, num_train_samples, num_validation_samples,
        ...
        Arguments:
            dataset_path {string} -- the path to save the data  数据保存路径
            num_train_samples {int} -- number of samples to use in training set (the sum of
                                       labeld + unlabeled train samples) 训练集使用的数据量
            num_validation_samples {int} -- number of samples to use in validation set 验证集
            num_labeled_samples {int} -- number of labeled samples to use 可用的标记数据量
            random_seed {int} -- seed to use 使用的种子
        """
    ...
    def __normalize_and_prepare_dataset(self, mat_dataset):
        ...
        # Convert it to zero mean and unit variance标准化数据
    ...
    def __download_and_extract_dataset(self):
        ...
        Returns:
            [np.array] -- normalized train images 标准化的训练图像
            [np.array] -- train labels  训练标签
            [np.array] -- normalized test images 标准化的测试图像
            [np.array] -- test labels  测试标签
        """
    ...
    def download_images_and_generate_tf_record(self):
        """ Main function of the class that allows generating and saving the tfrecords
            ...
            类的主要函数，允许产生和保存标记训练，无标记训练，验证，测试集的tf格式数据
        """

        # Create folder if needed 验证是否需要创建文件夹，文件夹存在就直接推出了
        if not os.path.exists(self._dataset_path):
            os.makedirs(self._dataset_path)
        else:  # Dataset already loaded
            return

        # Download and process dataset 下载和处理数据集（标准化到[0,1]）
        train_X, train_y, test_X, test_y = self.__download_and_extract_dataset()

        # Use the seed provided
        rng = np.random.RandomState(self._random_seed)  # 根据self._random_seed产生伪随机序列
